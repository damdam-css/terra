  ...
  const handleAnalyze = async (imgSrc?: string, name?: string) => {
    ...
    try {
      ...
      if (!response.ok || !json.success || !json.data) {
        throw new Error(
          json.error ||
            'AI tidak berhasil menganalisis foto. Coba foto ulang dengan objek terlihat jelas.'
        );
      }
      ...
      if (onScanComplete && !json.data.needsRetake) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7">
            {scanError && (
              <div className="mb-4 rounded-2xl border-2 border-red-700 bg-red-50 p-4 text-sm text-red-800">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-semibold">{scanError}</p>
                    <p className="mt-2 text-xs">
                      Coba foto satu objek sampah dari jarak sedang, jangan tertutup tangan,
                  ...
                  <div className="space-y-4 animate-fadeIn">
                    ...
                    <div className="bg-white border-3 border-[#1D3557] rounded-3xl p-6 shadow-[0_10px_0_0_#1D3557] relative">
                      ...
                      {scanResult.needsRetake && (
                        <div className="mb-5 rounded-2xl border-2 border-amber-700 bg-amber-50 p-4 text-sm text-amber-900">
                          <div className="flex items-start gap-2">
                            <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
                            <div>
                              <div className="font-heading font-black">
                                Identifikasi belum cukup yakin
                              </div>
                              <p className="mt-1 font-semibold">
                                AI tidak akan memaksakan jenis material kalau bukti visualnya kurang.
                                Ambil foto ulang dengan satu objek terlihat penuh dan fokus.
                              </p>
                              {scanResult.confidence !== undefined && (
                                <p className="mt-1 text-xs">
                                  Tingkat keyakinan visual: {Math.round(scanResult.confidence * 100)}%
                                </p>
                              )}
                              {scanResult.evidence && (
                                <p className="mt-2 text-xs">
                                  <strong>Bukti visual:</strong> {scanResult.evidence}
                                </p>
                              )}
                            </div>
                          </div>
                      ...
                      {scanResult.confidence !== undefined && !scanResult.needsRetake && (
                        <div className="mb-5 rounded-2xl border-2 border-emerald-700 bg-emerald-50 p-3 text-xs text-emerald-900">
                          <div className="font-heading font-black">
                            Keyakinan visual: {Math.round(scanResult.confidence * 100)}%
                          </div>
                          {scanResult.evidence && (
                            <div className="mt-1">
                              <strong>Bukti:</strong> {scanResult.evidence}
                            </div>
                          )}
                        </div>
                      )}
