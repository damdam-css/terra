...
export const Dashboard: React.FC<DashboardProps> = ({ role, fullName, email, onLogout }) => {
  ...
  const submitDeposit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      showNotice('Laporan berhasil dikirim. Tunggu konfirmasi petugas sebelum XP masuk.');
  ...
  const redeemReward = async (reward: Reward) => {
    if (xp < reward.points_cost) {
      showError(`XP kamu belum cukup. Reward ini membutuhkan ${reward.points_cost} XP.`);
      return;
    }
    if (reward.stock <= 0) {
    ...
    if (!window.confirm(`Ajukan penukaran ${reward.name} dengan ${reward.points_cost} XP?`)) return;
    ...
    try {
      ...
      showNotice('Pengajuan reward berhasil dibuat. XP baru dipotong setelah dikonfirmasi petugas.');
      await loadStudentData();
    } catch (err: any) {
  ...
  const verifyDeposit = async (deposit: Deposit, approved: boolean) => {
    ...
    try {
      ...
      showNotice(approved ? 'Setoran dikonfirmasi dan XP diberikan.' : 'Setoran ditolak.');
      await loadStaffData();
    } catch (err: any) {
  ...
  const verifyRedemption = async (redemption: Redemption, approved: boolean) => {
    ...
    try {
      ...
      showNotice(approved ? 'Penukaran reward dikonfirmasi.' : 'Penukaran reward ditolak.');
      await loadStaffData();
    } catch (err: any) {
  ...
  const Notice = () => (
    <>
      {error && (
        <div className="mb-5 rounded-2xl border-2 border-red-700 bg-red-50 p-4 text-sm font-semibold text-red-800 flex gap-2">
          <XCircle className="w-5 h-5 shrink-0" />
          <span>{error}</span>
        </div>
      )}
      {notice && (
        <div className="mb-5 rounded-2xl border-2 border-emerald-700 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800 flex gap-2">
      ...
      <>
        <div className="bg-[#BDE0FE] border-2 border-[#1D3557] rounded-3xl p-6 shadow-[0_6px_0_0_#1D3557] mb-6">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              ...
              <p className="mt-2 font-semibold">Setor sampah, kumpulkan XP, lalu tukarkan dengan reward.</p>
            </div>
            <div className="bg-[#FFF176] border-2 border-[#1D3557] rounded-2xl px-5 py-4 shadow-[0_4px_0_0_#1D3557] min-w-[180px]">
              <div className="text-[10px] font-black uppercase opacity-60">Earth-XP</div>
              <div className="font-heading font-black text-3xl">{loading ? '...' : xp.toLocaleString('id-ID')} XP</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <button onClick={() => setView('reward')} className="text-left bg-[#FFD6A5] border-2 border-[#1D3557] rounded-2xl p-5 shadow-[0_5px_0_0_#1D3557] hover:-translate-y-1 transition">
            ...
            <p className="text-sm opacity-65 mt-1">Tukarkan XP setelah laporanmu dikonfirmasi petugas.</p>
          </button>
        </div>
      ...
      <div className="grid lg:grid-cols-[420px_1fr] gap-6">
        <section className="bg-white border-2 border-[#1D3557] rounded-3xl p-5 shadow-[0_6px_0_0_#1D3557]">
          ...
          <form onSubmit={submitDeposit} className="space-y-4">
            <label className="block">
              ...
              <input value={depositForm.bankName} onChange={(e) => setDepositForm({ ...depositForm, bankName: e.target.value })} className="mt-1 w-full border-2 border-[#1D3557]/25 rounded-xl px-3 py-3 outline-none focus:border-[#1D3557]" />
            </label>
            <label className="block">
              <span className="text-xs font-black uppercase">Kategori</span>
              <select value={depositForm.category} onChange={(e) => setDepositForm({ ...depositForm, category: e.target.value as WasteCategory })} className="mt-1 w-full border-2 border-[#1D3557]/25 rounded-xl px-3 py-3 bg-white outline-none focus:border-[#1D3557]">
                {categoryOptions.map((category) => <option key={category}>{category}</option>)}
              </select>
            </label>
            <label className="block">
              <span className="text-xs font-black uppercase">Jenis / Material</span>
              <input value={depositForm.material} onChange={(e) => setDepositForm({ ...depositForm, material: e.target.value })} placeholder="Contoh: Botol PET" className="mt-1 w-full border-2 border-[#1D3557]/25 rounded-xl px-3 py-3 outline-none focus:border-[#1D3557]" />
            </label>
            <div className="block">
            ...
            <label className="block">
              ...
              <input type="number" min="0.01" max="1000" step="0.01" value={depositForm.weightKg} onChange={(e) => setDepositForm({ ...depositForm, weightKg: e.target.value })} placeholder="0.5" className="mt-1 w-full border-2 border-[#1D3557]/25 rounded-xl px-3 py-3 outline-none focus:border-[#1D3557]" />
            </label>
            <label className="block">
              <span className="text-xs font-black uppercase">Catatan (opsional)</span>
              <textarea value={depositForm.notes} onChange={(e) => setDepositForm({ ...depositForm, notes: e.target.value })} rows={3} placeholder="Keterangan tambahan" className="mt-1 w-full border-2 border-[#1D3557]/25 rounded-xl px-3 py-3 outline-none focus:border-[#1D3557] resize-none" />
            </label>
        ...
        <section className="bg-white border-2 border-[#1D3557] rounded-3xl shadow-[0_6px_0_0_#1D3557] overflow-hidden">
          <div className="p-5 border-b-2 border-[#1D3557]/10 flex items-center justify-between">
            <div><h2 className="font-heading font-black text-2xl">Riwayat Setoran</h2><p className="text-sm opacity-60">XP hanya masuk setelah petugas mengonfirmasi.</p></div>
              ...
              <div key={deposit.id} className="p-5">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  ...
                  <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-black ${statusClass(deposit.status)}`}>
                    ...
                    {deposit.status === 'approved' ? `+${deposit.points_awarded} XP` : deposit.status === 'rejected' ? 'Ditolak' : 'Menunggu'}
                  </span>
                </div>
  ...
  const StudentRewards = () => (
    <>
      ...
      <div className="bg-[#FFF176] border-2 border-[#1D3557] rounded-3xl p-5 shadow-[0_6px_0_0_#1D3557] mb-6 flex items-center justify-between gap-4">
        <div><div className="text-xs font-black uppercase opacity-60">Saldo tersedia</div><div className="font-heading font-black text-4xl">{xp.toLocaleString('id-ID')} XP</div></div>
        ...
        {rewards.map((reward) => (
          <div key={reward.id} className="bg-white border-2 border-[#1D3557] rounded-2xl p-5 shadow-[0_5px_0_0_#1D3557] flex flex-col">
            ...
            <div className="mt-4 flex items-center justify-between gap-2">
              <span className="font-heading font-black">{reward.points_cost} XP</span>
        ...
        <div className="divide-y-2 divide-[#1D3557]/10">
          {redemptions.map((redemption) => (
            <div key={redemption.id} className="p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex items-center gap-2"><span className="font-bold">{redemption.points_cost} XP</span><span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass(redemption.status)}`}>{redemption.status === 'approved' ? 'Disetujui' : redemption.status === 'rejected' ? 'Ditolak' : 'Menunggu'}</span></div>
            </div>
          ))}
  ...
  const VerificationPanel = () => (
    <>
      ...
      <div className="flex items-center justify-between mb-5">
        <div><h2 className="font-heading font-black text-3xl">Verifikasi</h2><p className="text-sm opacity-60">Periksa laporan siswa sebelum XP diberikan.</p></div>
        ...
        <div className="divide-y-2 divide-[#1D3557]/10">
          {redemptions.filter((r) => r.status === 'pending').map((redemption) => (
            <div key={redemption.id} className="p-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div><div className="font-black">{redemption.reward_name}</div><div className="text-sm mt-1">Biaya: <strong>{redemption.points_cost} XP</strong></div><div className="text-xs opacity-50 mt-1">{formatDate(redemption.created_at)}</div></div>
