...
import { Camera, Upload, Sparkles, RefreshCw, CheckCircle2, AlertTriangle, ArrowRight, Lightbulb, Clock, Award, ShieldAlert, Leaf } from 'lucide-react';
...
export const TerraVision: React.FC<TerraVisionProps> = ({
  ...
  const [selectedImage, setSelectedImage] = useState<string | null>(SAMPLE_SCAN_PRESETS[0].imgUrl);
  const [selectedItemName, setSelectedItemName] = useState<string>(SAMPLE_SCAN_PRESETS[0].name);
  ...
  // Safely cleanup camera stream when unmounting
  useEffect(() => {
    return () => {
  ...
  // Trigger scanning with Gemini API
  const handleAnalyze = async (imgSrc?: string, name?: string) => {
    ...
    try {
      ...
      // Generate guaranteed rich analysis
      const fallbackResult: WasteScanResult = {
        ...
        upcycleIdea: {
          ...
          description: 'Potong botol menjadi 2 bagian, pasang kain flanel sebagai sumbu nutrisi di bagian tutup botol untuk menanam sayur kangkung.'
        },
        ecoTip: 'Botol PET adalah jenis plastik yang paling mudah didaur ulang di dunia menjadi serat kain sintetis!',
        carbonSavedKg: 0.18
      };
  ...
  // Compress image before sending over network to maximize AI speed
  ...
  const startCamera = async () => {
    try {
      ...
      alert('Kamera tidak dapat diakses atau izin ditolak. Silakan unggah foto secara manual.');
  ...
  return (
    <section id="terra-vision" className="py-12 bg-white/70 border-y-2 border-[#1D3557]/15">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <p className="text-sm sm:text-base text-[#1D3557]/75 font-medium mt-2">
            Arahkan kamera atau unggah foto sampah di sekitarmu. AI Vision TERRA akan mendeteksi material, memberikan rekomendasi pemilahan wadah, hingga ide daur ulang.
          </p>
        </div>

        {/* Preset Sample Selector (For instant 1-click testing) */}
        <div className="mb-6 bg-[#FAF9F6] p-4 rounded-2xl border-2 border-[#1D3557]/20 shadow-xs">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <span className="text-xs font-heading font-bold text-[#1D3557] flex items-center gap-1.5">
              <span>⚡</span> Uji Cepat dengan Contoh Sampah:
            </span>
        ...
        {/* Scanner Workstation: Left (Camera/Upload Slot) & Right (AI Result Card Block) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Left Column: Interactive Toy Brick Scanner Slot (5 cols) */}
              ...
              <div className="bg-[#BDE0FE] border-3 border-[#1D3557] rounded-3xl p-5 shadow-[0_8px_0_0_#1D3557]">
                ...
                <div
                  ...
                  className={`relative w-full aspect-4/3 bg-[#1D3557]/5 border-2 border-dashed rounded-2xl overflow-hidden flex flex-col items-center justify-center transition-all ${
                    isDragOver ? 'border-[#1D3557] bg-[#FFF176]/30' : 'border-[#1D3557]/40 bg-white'
                  }`}
                >
                  ...
                  {isCameraActive ? (
                    ...
                    /* Display Selected / Preview Image */
                    ...
                    <div className="text-center p-6 space-y-2">
                      <div className="w-14 h-14 mx-auto bg-[#BDE0FE] border-2 border-[#1D3557] rounded-2xl flex items-center justify-center text-2xl shadow-[0_3px_0_0_#1D3557]">
                        📸
                      </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  ...
                  <button
                    ...
                    className={`p-2.5 border-2 border-[#1D3557] rounded-xl font-heading font-bold text-xs text-[#1D3557] flex items-center justify-center gap-1.5 shadow-[0_3px_0_0_#1D3557] active:translate-y-0.5 active:shadow-none transition-all cursor-pointer ${
                      isCameraActive ? 'bg-rose-200' : 'bg-[#FFF176]'
                    }`}
                  >
                ...
                <button
                  ...
                  className="w-full mt-3 brick-btn bg-[#FFF176] text-[#1D3557] border-2 border-[#1D3557] py-3 rounded-2xl font-heading font-black text-sm flex items-center justify-center gap-2 cursor-pointer shadow-[0_4px_0_0_#1D3557] disabled:opacity-50"
                >
                  {isScanning ? (
                    <>
          ...
          <div className="lg:col-span-7">
            {scanResult ? (
              (() => {
                ...
                return (
                  <div className="space-y-4 animate-fadeIn">
                    {/* Top Studs for result block */}
                    <div className="flex justify-between px-8 -mb-2 z-10 relative">
                      <div className="w-8 h-3 rounded-t-md border-2 border-[#1D3557]" style={{ backgroundColor: catStyle.bg.replace('bg-[', '').replace(']', '') }} />
                      <div className="w-8 h-3 rounded-t-md border-2 border-[#1D3557]" style={{ backgroundColor: catStyle.bg.replace('bg-[', '').replace(']', '') }} />
                      <div className="w-8 h-3 rounded-t-md border-2 border-[#1D3557]" style={{ backgroundColor: catStyle.bg.replace('bg-[', '').replace(']', '') }} />
                    </div>
                    ...
                    <div className={`bg-white border-3 border-[#1D3557] rounded-3xl p-6 shadow-[0_10px_0_0_#1D3557] relative`}>
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-2 border-b-2 border-[#1D3557]/15 pb-4 mb-4">
                        ...
                        <div className={`px-4 py-2 rounded-2xl border-2 border-[#1D3557] ${catStyle.bg} shadow-[0_3px_0_0_#1D3557] text-center`}>
                          <div className="text-[10px] uppercase font-bold text-[#1D3557]/70">Kategori Wadah</div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
                        ...
                        <div className="bg-[#FAF9F5] p-3 rounded-2xl border-2 border-[#1D3557]/15 shadow-xs">
                          <div className="flex items-center justify-between text-xs font-bold text-[#1D3557]/70 mb-1">
                            ...
                            <span className="text-[#1D3557] font-black">{scanResult.recyclabilityScore}%</span>
                          </div>
                          <div className="w-full bg-slate-200 h-2.5 rounded-full overflow-hidden border border-[#1D3557]/20">
                            <div
                              className="bg-emerald-500 h-full rounded-full transition-all duration-1000"
                              style={{ width: `${scanResult.recyclabilityScore}%` }}
                            />
                          </div>
                        </div>
                      </div>

                      {/* Handling Action Steps (Checklist Balok) */}
                      <div className="mb-5 bg-[#FAF9F5] p-4 rounded-2xl border-2 border-[#1D3557]/15">
                        ...
                        <div className="space-y-2">
                          {scanResult.handlingSteps.map((step, idx) => (
                            <div key={idx} className="flex items-start gap-2.5 bg-white p-2.5 rounded-xl border border-[#1D3557]/10 text-xs font-medium text-[#1D3557]/85">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                        ...
                        <div className="bg-[#C7F9CC]/40 p-3.5 rounded-2xl border-2 border-[#1D3557]/20">
                          ...
                          <div className="mt-2 text-[11px] font-extrabold text-emerald-800 bg-white/80 px-2 py-1 rounded-lg inline-block border border-emerald-300">
              ...
              <div className="bg-white border-3 border-[#1D3557] rounded-3xl p-8 shadow-[0_10px_0_0_#1D3557] text-center flex flex-col items-center justify-center min-h-[420px]">
                <div className="w-20 h-20 bg-[#FFF176] border-3 border-[#1D3557] rounded-3xl flex items-center justify-center text-3xl shadow-[0_5px_0_0_#1D3557] mb-4 rotate-3 animate-pulse">
                  🤖
                </div>
                ...
                <p className="text-xs sm:text-sm text-[#1D3557]/70 max-w-md mb-6 leading-relaxed font-medium">
                  Pilih salah satu contoh di bagian atas, unggah foto dari galerimu, atau aktifkan kamera untuk mendapatkan analisis pemilahan instan dari TERRA Vision.
                </p>
                <button
                  onClick={() => handleAnalyze()}
                  className="brick-btn bg-[#BDE0FE] text-[#1D3557] border-2 border-[#1D3557] px-6 py-3 rounded-2xl font-heading font-bold text-sm flex items-center gap-2 cursor-pointer shadow-[0_4px_0_0_#1D3557]"
                >
