...
async function generateContentWithFallback(
  ...
  for (const model of candidateModels) {
    try {
      ...
      // Only transient capacity/model-availability errors should fall back.
...
async function startServer() {
  ...
  async function ensureStorageBucket(name: string, isPublic: boolean, fileSizeLimit: string) {
    if (!supabaseAdmin) return;
    const { data, error } = await supabaseAdmin.storage.getBucket(name);
    if (data) return;
    if (error && !String(error.message || '').toLowerCase().includes('not found')) return;
    ...
    if (createError && !String(createError.message || '').toLowerCase().includes('already exists')) {
      console.warn(`Gagal membuat bucket ${name}:`, createError.message);
    }
  }
  ...
  void ensureStorageBucket(profilePhotoBucket, true, "2MB");
  ...
  app.post("/api/admin/users/:id/delete", async (req, res) => {
    ...
    const { data: authTarget, error: authTargetError } = await supabaseAdmin!.auth.admin.getUserById(targetId);
    if (authTargetError || !authTarget?.user) return res.status(404).json({ error: "Akun Auth tidak ditemukan." });
  ...
  app.post("/api/admin/users/:id/reset-password", async (req, res) => {
    const staff = await requireStaff(req, res);
    if (!staff) return;
    if (staff.profile.role !== "admin") return res.status(403).json({ error: "Hanya admin yang dapat mereset password." });

    const targetId = req.params.id;
    if (targetId === staff.user.id) return res.status(400).json({ error: "Gunakan pengaturan akun sendiri untuk mengganti password admin." });
  ...
  // Update nama tampilan milik akun yang sedang login.
  app.put("/api/profile/name", async (req, res) => {
    ...
    if (fullName.length < 2 || fullName.length > 60) {
      return res.status(400).json({ error: "Nama harus 2-60 karakter." });
    }
    ...
    res.json({ success: true, full_name: updated.full_name });
  });
  ...
  // It reads the same production database for every device.
  app.get("/api/leaderboard", async (_req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin
        .from("point_wallets")
        .select("user_id, balance, profiles!inner(full_name, role, avatar_url, is_blocked)")
        .eq("profiles.role", "siswa")
      ...
      const leaderboard = (data || []).map((row: any, index: number) => ({
        id: row.user_id,
        full_name: row.profiles?.full_name || "Siswa",
  ...
  app.post("/api/staff/redemptions/:id/pickup", async (req, res) => {
    const staff = await requireStaff(req, res);
    if (!staff) return;

    const { data, error } = await supabaseAdmin!.rpc("mark_reward_redemption_picked_up", {
      p_redemption_id: req.params.id,
      p_staff_id: staff.user.id,
    });
...
- impact: string (manfaat positif lingkungan & kegunaan barang baru)` ,
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`TERRA App running on port ${PORT}`);
  });
}
